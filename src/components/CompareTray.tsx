...
const ATTRS: Attr[] = [
  ...
  },
  {
    ...
    highlight: 'max',
    numeric: (r) => r.chip.memoryBandwidthGBps ?? 0,
  },
...
export default function CompareTray({
  ...
  const best = (attr: Attr): number | null => {
    ...
    const vals = rows.map(attr.numeric)
    // 값이 전부 같으면 강조할 우열이 없다. 전부 굵게 칠하면 노이즈만 된다.
  ...
  return (
    ...
    >
      {open && (
        <div
          className="max-h-[65vh] overflow-auto border-b"
              ...
              <tbody>
                {visible.map((attr) => {
                  ...
                  return (
                    ...
                    >
                      <td
                        className="sticky left-0 z-10 px-2 py-1.5 whitespace-nowrap"
                        style={{ background: 'var(--surface)', color: 'var(--muted)' }}
                      >
                        {attr.label}
                      </td>
                      {rows.map((r) => {
                        const isBest =
                          bestVal !== null && attr.numeric!(r) === bestVal
                        return (
                          <td
                            key={r.id}
