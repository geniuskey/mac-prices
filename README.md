...
기본 구성과 BTO 옵션으로 만들 수 있는 메모리·저장장치 조합은 각각 독립된 row로
표시하므로, 옵션별 가격 차이를 그대로 비교할 수 있습니다.
...
- **벤치마크 참고값** — 칩별 Geekbench 6 CPU·Metal 점수와 원본 제출 링크 확인
...
| 항목 | 현재 상태 |
|---|---|
| 기본 구성 정가 | **39/43 대조** — M5 Max는 Apple 한국 스토어, 나머지는 출시 보도자료·언론 기사(`press`) 기준 |
| 현재가가 추정치인 구성 | **79/687** — 2026-06-25 전 라인 인상과 옵션 조합으로 계산한 구성 |
| BTO 업그레이드 단가 | **1/43** — M5 Max는 맞춤 구성별 확인값 |

M5 Max 맞춤 구성과 일부 기본가는 apple.com/kr에서 직접 확인했습니다. 나머지
**동일 조건 비교에 표시되는 업그레이드 금액은 추정값**입니다. 2026년 6월 Apple 이 메모리 업그레이드 가격을
